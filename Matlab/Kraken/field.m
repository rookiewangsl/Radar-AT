...
if ( NProf == 1 )               % Range-independent case
   ...
   isz = find( abs( Modes.z - zs ) < eps( max( Modes.z ) ) );    % index of source depth
   if ( isempty( isz ) )
      errordlg( 'Modes not tabulated at source depth' )
   end
   
   isz = isz( 1 );
   C   = Modes.phi( isz, : ).';    % column vector with modal weights
   ...
   irzvec = zeros( 1, length( Pos.r.z ) );
   for ii = 1 : length( Pos.r.z )
      zr  = Pos.r.z( ii );
      irz = find( Modes.z >= zr );	% index of source depth
      if ( numel( irz ) == 0 )
          irz( 1 ) = length( Pos.r.z );
      end
      irzvec( ii ) = irz( 1 );
   end
   phiR = Modes.phi( irzvec, : );
   ...
   if ( Opt(2:2) == 'C' )       % Range-dependent case
      ...
      pressure = evalcm( FileRoot, Pos, rProf, NProf, rz, Nrz, R, Nr, MLimit, Opt );
   else
      % Adiabatic mode theory
      pressure = evalad( FileRoot, Pos, rProf, NProf, rz, Nrz, R, Nr, M, Opt );
   end
end
