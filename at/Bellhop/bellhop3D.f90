  ...
  Source_z: DO isz = 1, Pos%Nsz         ! loop over source z-coordinate
     ...
     Source_x: DO isx = 1, Pos%Nsx      ! loop over source x-coordinate
        ...
        Source_y: DO isy = 1, Pos%Nsy   ! loop over source y-coordinate
           ...
           AzimuthalAngle: DO ibeta = 1, Angles%Nbeta   ! this is also the receiver bearing angle for a 2D run 25, 65, 2 ! 
...
CONTAINS
  SUBROUTINE CurvatureCorrection
    ...
    ! correct p-q due to jumps in the gradient of the sound speed
     ...
     tBdry = tBdry / NORM2b( tBdry )        ! unit boundary tangent
...
SUBROUTINE Reflect3D( is, HS, BotTop, nBdry, z_xx, z_xy, z_yy, kappa_xx, kappa_xy, kappa_yy, RefC, Npts )
  ...
  REAL    (KIND=8) :: Phi, z_xx, z_xy, z_yy, kappa_xx, kappa_xy, kappa_yy, t_rot( 2 ), n_rot( 2 )
  REAL    (KIND=8) :: RotMat( 2, 2 ), kappaMat( 2, 2 ), DMat( 2, 2 ), DMatTemp( 2, 2 )

  is  = is + 1
  is1 = is + 1

  !CALL ConeFormulas(    z_xx, z_xy, z_yy, nBdry )   ! analytic formulas for the curvature of the seamount
...
!!$  END IF
  ...
  CASE ( 'A', 'G' )            ! half-space
     ...
     IF ( ABS( RadDeg * ATAN2( nBdry( 3 ), NORM2b( nBdry( 1 : 2 ) ) ) ) < 60 ) THEN
        Refl = 1
        ! WRITE( *, * ) nBdry, ABS( RadDeg * ATAN2( nBdry( 3 ), NORM2b( nBdry( 1 : 2 ) ) ) )
     END IF
...
CONTAINS
  ...
  SUBROUTINE ConeFormulas( z_xx, z_xy, z_yy, nBdry )
    ...
    REAL (KIND=8) :: theta, Radius, x, y, RLen
