MODULE sspmod
  ...
  IMPLICIT NONE
  SAVE
  INTEGER, PARAMETER :: MaxSSP = 10001, MaxBioLayers = 20
  ...
  TYPE SSPStructure
    ...
    COMPLEX (KIND=8)     :: c0( MaxSSP ), c1( MaxSSP ), c2( MaxSSP ), c3( MaxSSP )   ! for PCHIP coefs.
...
CONTAINS
  ...
  SUBROUTINE EvaluateSSP( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task )
    ...
    CASE ( 'P' )
       CALL PCHIP(  x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task )
    CASE ( 'S' )
  ...
  SUBROUTINE PCHIP( x, c, cimag, gradc, crr, crz, czz, rho, Freq, Task )

    ! Piecewise Cubic Hermite Interpolating Polynomial of SSP data
    ! see http://www.maths.nuigalway.ie/~niall/teaching/Archive/1617/MA378/2-3-PCHIP.pdf
    ! trivial mod to handle a complex function (that might not be precisely correct)
    ...
    REAL     (KIND=8), INTENT( OUT ) :: c, cimag, gradc( 2 ), crr, crz, czz, rho ! sound speed and its derivatives
    INTEGER           :: iz
    REAL     (KIND=8) :: Depth, W, h, xt, W1, W2, h1, h2
    COMPLEX  (KIND=8) :: c_cmplx, f1, f2, f1prime, f2prime
    
    IF ( Task == 'INI' ) THEN   ! read in SSP data
       Depth     = x( 2 )
       CALL ReadSSP( Depth, Freq, ENVFile, PRTFile )

       ! precompute nodal derivatives
       ! The vector SSP%c0 duplicates SSP%c
       ! The vector SSP%c1 holds the first derivative at a node

       f1prime = SSP%cz( 1 )   ! forward difference for first point in depth
       ...
       DO iSegz = 2, SSP%NPts - 1
          h2  = SSP%z( iSegz + 1 ) - SSP%z( iSegz )

          ! The code commented out below implements a standard version of PCHIP
          ! There might be a coding error, but it was producing a sort of caustic beam in at/tests/Noise/ATOC
          ! It has been replaced with another PCHIP that does not have the monotonic property
          ...
          !IF ( REAL( SSP%cz( iSegz - 1 ) * SSP%cz( iSegz ) ) > 0 ) THEN
             !f2prime = ( w1 + w2 ) / ( w1 / SSP%cz( iSegz - 1 ) + w2 / SSP%cz( iSegz ) )   ! derivative from geo mean between adjacent segments

             ! Following mean is not an 'optimal' approximation if the grid is irregular
             f2prime = 0.5 * ( SSP%cz( iSegz - 1 ) + SSP%cz( iSegz ) )   ! derivative from mean between adjacent segments
          !END IF

          SSP%c0( iSegz ) = SSP%c( iSegz )
          SSP%c1( iSegz ) = f2prime
          h1      = h2
          f1prime = f2prime
       END DO

       SSP%c0( SSP%NPts ) = SSP%c(  SSP%Npts     )
       SSP%c1( SSP%Npts ) = SSP%cz( SSP%Npts - 1 )   ! backward difference for last point in depth

       ! evaluate the coefficients of the cubic
       DO iSegz = 1, SSP%NPts - 1
          h  = SSP%z( iSegz + 1 ) - SSP%z( iSegz )

          f1 = SSP%c( iSegz )
          f2 = SSP%c( iSegz + 1 )

          f1prime = SSP%c1( iSegz )
          f2prime = SSP%c1( iSegz + 1 )

          SSP%c2( iSegz ) = 3 * ( f2 - f1 ) / h**2 - ( f2prime + 2 * f1prime ) / h
          SSP%c3( iSegz ) = ( f2prime + f1prime ) / h**2 - 2 * ( f2 - f1 ) / h**3
       END DO
    ...
    ELSE                        ! return SSP info
       ...
       xt = x( 2 ) - SSP%z( iSegz )
       c_cmplx = SSP%c0( iSegz ) + SSP%c1( iSegz ) * xt + SSP%c2( iSegz ) * xt**2 + SSP%c3( iSegz ) * xt**3   ! here's the Hermite cubic
       !c_cmplx = SSP%c0( iSegz ) + xt * ( SSP%c1( iSegz ) + xt * ( SSP%c2( iSegz ) + xt * SSP%c3( iSegz ) ) )  ! here's the Hermite cubic
       
       c     = REAL(  c_cmplx )
       cimag = AIMAG( c_cmplx )
       gradc = [ 0.0D0, REAL( SSP%c1( iSegz ) + 2 * SSP%c2( iSegz ) * xt + 3 * SSP%c3( iSegz ) * xt**2 ) ]
       crr   = 0.0d0
       crz   = 0.0d0
       czz   = REAL( 2 * SSP%c2( iSegz ) + 6 * SSP%c3( iSegz ) * xt )
