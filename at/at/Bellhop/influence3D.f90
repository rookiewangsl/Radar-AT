SUBROUTINE Influence3DGeoHatRayCen( xs, alpha, beta, Dalpha, Dbeta, P )
  ...
  REAL    (KIND=8) :: nA, nB, mA, mB, rA, rB, &
       W, s, DS, deltaA, deltaB, rayt( 3 ), t_rcvr( 2, Pos%Ntheta ), &
       L1, L2, m, n, a, b, Amp, const, KMAHphase( Beam%Nsteps ), phaseInt, DetQint, Ratio1
  ...
  REAL    (KIND=8) :: q_tilde( 2 ), q_hat( 2 ), DetQ( Beam%Nsteps ), &
       dq_tilde( 2 ), dq_hat( 2 ), &
  ...
  ReceiverDepths: DO id = 1, Pos%Nrz
     ...
     xt( 3, : ) = ray3D( 1 : Beam%Nsteps )%x( 3 ) - Pos%rz( id )
     DO is = 1, Beam%Nsteps
           ...
           IF ( irA /= irB .AND. irA /= 0 .AND. irB /= 0 .AND. &
              ...
              Ranges: DO ir = irA + 1 - II, irB + II, SIGN( 1, irB - irA )
                 ...
                 CASE ( 'A', 'a' ) ! arrivals
                    ...
                    RcvrDeclAngle  = RadDeg * ATAN2( rayt( 3 ), NORM2b( rayt( 1 : 2 ) ) )
...
SUBROUTINE Influence3DGeoHatCart( xs, alpha, beta, Dalpha, Dbeta, P, x_rcvrMat, t_rcvr )
  ...
  REAL    ( KIND=8 ) :: s, rlen, &
       ...
       L1, L2, L_z, L_diag, e_theta( 3 ), m, n, m_prime, a, b, zMin, zMax, &
       Amp, const, phase, phaseInt, Det_Q, Det_Qold, DetQint, Ratio1, &
     ...
     IF ( ABS( rB - rA ) > 1.0D3 * SPACING( rA ) ) THEN   ! jump to next step if duplicate point
        ...
        IF ( is == 2 ) THEN
           ...
           ELSE                  ! ray is moving left
              ir = Pos%Nrr        ! index all the way to the right
           END IF
        END IF
        ...
        IF ( rlen > 1.0D3 * SPACING( ray3D( is )%x( 1 ) ) ) THEN  ! Make sure this is not a duplicate point
           ...
           L1 = MAX( NORM2b( ray3D( is - 1 )%q_tilde ), NORM2b( ray3D( is )%q_tilde ) ) ! beamwidths
           L2 = MAX( NORM2b( ray3D( is - 1 )%q_hat   ), NORM2b( ray3D( is )%q_hat   ) )
              ...
              IF ( Pos%rr( ir ) >= MIN( rA, rB ) .AND. Pos%rr( ir ) < MAX( rA, rB ) ) THEN
                 ...
                 Radials: DO itheta = 1, Pos%Ntheta   ! Loop over radials of receiver line
                    ...
                    ReceiverDepths: DO id = 1, Pos%Nrz
                       ...
                       L1 = NORM2b( q_tilde )  ! beamwidth is length of the vector
                       L2 = NORM2b( q_hat   )  ! beamwidth
...
SUBROUTINE Influence3DGeoGaussianCart( xs, alpha, beta, Dalpha, Dbeta, P, x_rcvrMat, t_rcvr )
  ...
  COMPLEX        , INTENT( OUT ) :: P( Pos%Ntheta, Pos%Nrz, Pos%Nrr )   ! complex pressure
  INTEGER            :: itheta, ir, is, id, irT( 1 ), irTT
  REAL    ( KIND=8 ) :: s, rlen, a, b, &
       rA, rB, W, DS, x_ray( 3 ), rayt( 3 ), n_ray_z( 3 ), n_ray_theta(3 ), &
       e1( 3 ), e2( 3 ), x_rcvr( 3 ), x_rcvr_ray( 3 ), &
       L1, L2, L1_stint, L2_stint, L_z, L_diag, e_theta( 3 ), m, n, m_prime, zMin, zMax, &
       Amp, const, phase, phaseInt, Det_Q, Det_Qold, DetQint, Ratio1, &
       q_tilde( 2 ), q_hat( 2 ), q_tilde_hat( 2 ), dq_tilde( 2 ), dq_hat( 2 ), &
       SrcDeclAngle, SrcAzimAngle, RcvrDeclAngle, RcvrAzimAngle, &
       lambda
  COMPLEX ( KIND=8 ) :: dtau, delay
