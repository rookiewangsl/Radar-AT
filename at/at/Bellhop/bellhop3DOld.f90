     ...
     Source_x: DO isx = 1, Pos%Nsx      ! loop over source x-coordinate
        ...
        Source_y: DO isy = 1, Pos%Nsy   ! loop over source y-coordinate
           ...
           CASE ( 'C', 'S', 'I' )   ! TL calculation
              ...
              END DO
              CASE ( 'A' )             ! arrivals calculation, ascii
                 CALL WriteArrivalsASCII3D(  Pos%rr, Pos%Ntheta, Nrz_per_range, Pos%Nrr, Beam%RunType( 4:4 ) )
              CASE ( 'a' )             ! arrivals calculation, binary
                 CALL WriteArrivalsBinary3D( Pos%rr, Pos%Ntheta, Nrz_per_range, Pos%Nrr, Beam%RunType( 4:4 ) )
           END SELECT
        END DO Source_y
     END DO Source_x
  END DO Source_z
...
SUBROUTINE TraceRay2D( xs, alpha, beta, Amp0 )
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     IF      ( DistBegTop > 0.0d0 .AND. DistEndTop <= 0.0d0 ) THEN  ! test top reflection
        ...
        CALL Reflect2D( is, Bdry%Top%HS, 'TOP', TopnInt, z_xx, z_xy, z_yy, RTop, NTopPTS, xs, tradial )
     ...
     ELSE IF ( DistBegBot > 0.0d0 .AND. DistEndBot <= 0.0d0 ) THEN  ! test bottom reflection
        ...
        CALL Reflect2D( is, Bdry%Bot%HS, 'BOT', BotnInt, z_xx, z_xy, z_yy, RBot, NBotPTS, xs, tradial )
...
SUBROUTINE Reflect2D( is, HS, BotTop, nBdry3d, z_xx, z_xy, z_yy, RefC, Npts, xs, tradial )
  ...
  REAL     (KIND=8) :: Radius, z_xx, z_xy, z_yy, x, y, &
  ...
  !CALL ConeFormulas( z_xx, z_xy, z_yy, nBdry )   ! special case of a conical seamount
  kappa = ( z_xx * tradial( 1 ) ** 2 + 2 * z_xy * tradial( 1 ) * tradial( 2 ) + z_yy * tradial( 2 ) ** 2 ) / 2
  IF ( BotTop == 'TOP' ) kappa = -kappa

  nBdry = nBdry / NORM2b( nBdry )
  ...
  CONTAINS
 SUBROUTINE ConeFormulas( z_xx, z_xy, z_yy, nBdry )
...
SUBROUTINE TraceRay3D( xs, alpha, beta, epsilon, Amp0 )
  ...
  Stepping: DO istep = 1, MaxN - 1
     ...
     IF      ( DistBegTop > 0.0d0 .AND. DistEndTop <= 0.0d0 ) THEN  ! test top reflection
        ...
        CALL Reflect3D( is, Bdry%Top%HS, 'TOP', TopnInt, z_xx, z_xy, z_yy, RTop, NTopPTS )
     ...
     ELSE IF ( DistBegBot > 0.0d0 .AND. DistEndBot <= 0.0d0 ) THEN  ! test bottom reflection
        ...
        CALL Reflect3D( is, Bdry%Bot%HS, 'BOT', BotnInt, z_xx, z_xy, z_yy, RBot, NBotPTS )
...
SUBROUTINE Step3D( ray0, ray2 )
  ...
  IF ( iSegx /= iSegx0 .OR. &
     ...
     tBdry = tBdry / NORM2b( tBdry )        ! unit normal
     ...
     rayn2 = rayn2 / NORM2b( rayn2 )        ! unit normal
...
SUBROUTINE Reflect3D( is, HS, BotTop, nBdry, z_xx, z_xy, z_yy, RefC, Npts )
  ...
  REAL    (KIND=8) :: Phi, z_xx, z_xy, z_yy, t_rot( 2 ), n_rot( 2 )
  REAL    (KIND=8) :: RotMat( 2, 2 ), kappaMat( 2, 2 ), DMat( 2, 2 ), DMatTemp( 2, 2 )

  is  = is + 1
  is1 = is + 1

  !CALL ConeFormulas( z_xx, z_xy, z_yy, nBdry )   ! analytic formulas for the curviture of the seamount
  ...
  t_rot = rayt(  1 : 2 ) / NORM2b( rayt(  1 : 2 ) )
  ...
  DMatTemp = MATMUL( kappaMat, RotMat )
...
!!$  END IF
  ...
  ! Also the DMat differs by a factor of 2 from Cerveny's
  RM = Tg / Th   ! this is tan( alpha ) where alpha is the angle of incidence

  R1 = +2 / c ** 2 * DMat( 1, 1 ) / Th + RM * ( 2 * cn1jump - RM * csjump ) / c ** 2
  R2 = +2 / c *      DMat( 1, 2 )      + RM * cn2jump / c ** 2
  R3 = +2 *          DMat( 2, 2 ) * Th
  ...
  p_tilde_in = RotMat( 1, 1 ) * ray3D( is )%p_tilde + RotMat( 1, 2 ) * ray3D( is )%p_hat
  p_hat_in   = RotMat( 2, 1 ) * ray3D( is )%p_tilde + RotMat( 2, 2 ) * ray3D( is )%p_hat

  q_tilde_in = RotMat( 1, 1 ) * ray3D( is )%q_tilde + RotMat( 1, 2 ) * ray3D( is )%q_hat
  q_hat_in   = RotMat( 2, 1 ) * ray3D( is )%q_tilde + RotMat( 2, 2 ) * ray3D( is )%q_hat
  ...
  ray3d( is1 )%p_tilde = RotMat( 1, 1 ) * p_tilde_out + RotMat( 2, 1 ) * p_hat_out
  ray3d( is1 )%p_hat   = RotMat( 1, 2 ) * p_tilde_out + RotMat( 2, 2 ) * p_hat_out
...
CONTAINS
  SUBROUTINE CalcTangent_Normals( ray3Dt, nBdry, rayt, rayn1, rayn2 )
    ...
    rayn2 = -cross_product( rayt,      nBdry )         ! ray tangent x boundary normal gives refl. plane normal
    rayn2 = rayn2       / NORM2b( rayn2      )         ! unit normal to refl. plane
    rayn1 = -cross_product( rayt,       rayn2      )   ! ray tangent x refl. plane normal is first ray normal
