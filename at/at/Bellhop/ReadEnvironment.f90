SUBROUTINE ReadEnvironment( FileRoot, ThreeD )
  ...
  ! CALL ReadSzRz( ENVFile, PRTFile, ZMin + 100 * SPACING( ZMin ), ZMax - 100 * SPACING( ZMax ) )   ! not sure why I had this
  CALL ReadSzRz( ENVFile, PRTFile, ZMin, ZMax )
...
!**********************************************************************!
  ...
  CASE ( 'I' )
     ...
     IF ( Pos%Nrz /= Pos%Nrr ) CALL ERROUT( PRTFile, 'F', 'READIN', 'Irregular grid option selected with Nrz not equal to Nr' )
     PlotType = 'irregular '
  CASE DEFAULT
