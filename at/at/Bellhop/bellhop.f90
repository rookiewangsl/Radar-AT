...
SUBROUTINE BellhopCore
  ...
  IF ( btyType( 2 : 2 ) == 'L' ) THEN
     DO iSeg = 1, NbtyPts
        Bot( iSeg )%HS%cp = CRCI( 1e20, Bot( iSeg )%HS%alphaR, Bot( iSeg )%HS%alphaI, freq, freq, 'W ', &
             betaPowerLaw, ft, bio, NBioLayers )   ! compressional wave speed 
        Bot( iSeg )%HS%cs = CRCI( 1e20, Bot( iSeg )%HS%betaR,  Bot( iSeg )%HS%betaI,  freq, freq, 'W ', &
             betaPowerLaw, ft, bio, NBioLayers )   ! shear         wave speed
     END DO
  END IF
