SUBROUTINE InfluenceCervenyRayCen( U, epsilon, alpha, iBeamWindow2, RadMax )
  ...
  RcvrDepths: DO id = 1, Nrz_per_range
     zR = Pos%rz( id )
...
SUBROUTINE InfluenceGeoHatRayCen( U, alpha, dalpha )
  ...
  REAL    (KIND=8) :: nA, nB, rA, rB, W, zr, znv, rnv, ratio1, n, Amp, phase, const, phaseInt, &
       q0, q, qold, SrcDeclAngle, RcvrDeclAngle, dalpha, L, dq( Beam%Nsteps - 1 )
...
SUBROUTINE InfluenceGeoHatCart( U, alpha, Dalpha )
  ...
  REAL    (KIND=8) :: x_ray( 2 ), rayt( 2 ), rayn( 2 ), x_rcvr( 2 ), s, n, phaseInt, &
                      ...
                      Amp, phase, dqds, q, SrcDeclAngle, RcvrDeclAngle
        ...
        IF ( Pos%rr( ir ) >= MIN( rA, rB ) .AND. Pos%rr( ir ) < MAX( rA, rB ) ) THEN
           RcvrDepths: DO id = 1, Nrz_per_range
              IF ( Beam%RunType( 5 : 5 ) == 'I' ) THEN
                 IF ( Pos%rz( ir ) < zmin .OR. Pos%rz( ir ) > zmax ) CYCLE RcvrDepths
                 x_rcvr = [ Pos%rr( ir ), Pos%rz( ir ) ]   ! irregular   grid
              ELSE
                 IF ( Pos%rz( id ) < zmin .OR. Pos%rz( id ) > zmax ) CYCLE RcvrDepths
                 x_rcvr = [ Pos%rr( ir ), Pos%rz( id ) ]   ! rectilinear grid
              END IF
...
SUBROUTINE InfluenceGeoGaussianCart( U, alpha, Dalpha )
  ...
  REAL    (KIND=8) :: x_ray( 2 ), rayt( 2 ), rayn( 2 ), x_rcvr( 2 ), s, n, phaseInt, &
                      ...
                      Amp, phase, dqds, SrcDeclAngle, RcvrDeclAngle
  COMPLEX (KIND=8) :: delay, dtauds
...
SUBROUTINE InfluenceSGB( U, alpha, Dalpha )
  ...
  REAL (KIND=8)     :: x( 2 ), rayt( 2 ), A, beta, cn, CPA, deltaz, DS, phase, q, qOld, rA,rB, Ratio1, &
                       sint, SX1, thet, W, SrcDeclAngle, alpha, Dalpha
  COMPLEX           :: U( Nrz_per_range, Pos%Nrr )
  ...
  Stepping: DO is = 2, Beam%Nsteps
     ...
     RcvrRanges: DO WHILE ( ABS( rB - rA ) > 1.0D3 * SPACING( rA ) .AND. rB > Pos%rr( ir ) )   ! Loop over bracketted receiver ranges
        ...
        RcvrDepths: DO id = 1, Nrz_per_range
           deltaz =  Pos%rz( id ) - x( 2 )   ! ray to rcvr distance
           ! Adeltaz    = ABS( deltaz )
