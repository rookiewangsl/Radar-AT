...
CONTAINS
  ...
  SUBROUTINE EvaluateSSP( cP, cS, rho, Medium, N1, Freq, Task, ENVFile, PRTFile )
    ...
    CASE ( 'P' )  !  C-linear profile option 
       CALL PCHIP(  cP, cS, rho, Medium, N1, Freq, Task, ENVFile, PRTFile )
  ...
  SUBROUTINE PCHIP( cP, cS, rho, Medium, N1, Freq, Task, ENVFile, PRTFile  )

    ! Tabulate cP, cS, rho for specified Medium

    ! Uses Piecewise Cubid Hermite Interpolating Polynomial for P and S-wave speeds
    ! Uses piecewise-linear segments for density
    ! See also the code in at/Bellhop
    ! This version is a simpler PCHIP that does not preserve monotonicity
    ...
    CHARACTER (LEN=8), INTENT(IN)  :: Task
    INTEGER                        :: N, N1
    REAL     (KIND=8)              :: R, hSSP, xt
    COMPLEX  (KIND=8)              :: f1, f2, f1prime, f2prime, c0, c1, c2, c3
    ...
    ELSE   ! Task = 'TABULATE'
       ...
       DO iz = 1, N1
          ...
          IF ( iSSP == 1 ) THEN
             f1prime = ( SSP%cp( 2        ) -  SSP%cp( 1            ) ) / hSSP     ! forward difference for first point in depth
          ELSE
             f1prime = ( SSP%cp( iSSP + 1 ) -  SSP%cp( iSSP - 1     ) ) / hSSP / 2 ! centered difference
          END IF

          IF ( iSSP == SSP%NPts( Medium ) - 1 ) THEN
             f2prime = ( SSP%cp( SSP%NPts( Medium ) ) -  SSP%cp( SSP%NPts( Medium ) - 1 ) ) / hSSP  ! backward difference for last point in depth
          ELSE
             f2prime = ( SSP%cp( iSSP + 2 ) -  SSP%cp( iSSP         ) ) / hSSP / 2  ! centered difference
          END IF

          c0 = f1
          c1 = f1prime
          c2 = 3 * ( f2 - f1 ) / hSSP**2 - ( f2prime + 2 * f1prime ) / hSSP
          c3 = ( f2prime + f1prime ) / hSSP**2 - 2 * ( f2 - f1 ) / hSSP**3

          xt = z - SSP%z( iSSP )
          cP( iz ) = c0 + c1 * xt + c2 * xt**2 + c3 * xt**3   ! here's the Hermite cubic
          !c_cmplx = c0 + xt * ( c1 + xt * ( c2 + xt * c3 ) )  ! here's the Hermite cubic
          ...
          IF ( iSSP == 1 ) THEN
             f1prime = ( SSP%cs( 2        ) -  SSP%cs( 1            ) ) / hSSP       ! forward difference for first point in depth
          ELSE
             f1prime = ( SSP%cs( iSSP + 1 ) -  SSP%cs( iSSP - 1     ) ) / hSSP / 2   ! centered difference
          END IF

          IF ( iSSP == SSP%NPts( Medium ) - 1 ) THEN
             f2prime = ( SSP%cs( SSP%NPts( Medium ) ) -  SSP%cs( SSP%NPts( Medium ) - 1 )  ) / hSSP  ! backward difference for last point in depth
          ELSE
             f2prime = ( SSP%cs( iSSP + 2 ) -  SSP%cs( iSSP         ) ) / hSSP / 2   ! centered difference
          END IF
          ...
          xt = z - SSP%z( iSSP )
          cS( iz ) = c0 + c1 * xt + c2 * xt**2 + c3 * xt**3   ! here's the Hermite cubic
          !c_cmplx = c0 + xt * ( c1 + xt * ( c2 + xt * c3 ) )  ! here's the Hermite cubic
